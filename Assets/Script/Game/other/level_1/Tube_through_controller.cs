...
public class Tube_through_controller : MonoBehaviour
{
    ...
    private IEnumerator TranslateBigCat()
    {
        ...
        {
            Big_cat.transform.Translate(Vector3.right * 40f * Time.deltaTime);
            Debug.Log(Big_cat.transform.position.x);
            yield return null;
        }
        while (Big_cat.transform.position.y < gameObject.transform.position.y + 8.5f)
        {
        ...
        while (Big_cat.transform.position.x < gameObject.transform.position.x + 8)
        {
