...
public class B_TubeChangeLevel_up : MonoBehaviour
{
    ...
    private IEnumerator CatTurn()
    {
        ...
        Y_distance = (tubeCat.transform.position.y) - (Turn_2.transform.position.y);

        while (X_distance > 0.5f || X_distance < -0.5f)
        {
            tubeCat.transform.rotation = Quaternion.Euler(0f, 0f, 0f);
        ...
        {
            tubeCat.transform.rotation = Quaternion.Euler(0f, 0f, 90f);
            tubeCat.transform.Translate(Vector3.right * (tubeSpeed + 10f) * Time.deltaTime);
            Y_distance = (tubeCat.transform.position.y) - (Turn_2.transform.position.y);
            yield return null;
        }

        tubeCat.transform.position = Turn_2.transform.position;

        X_distance = (tubeCat.transform.position.x) - (Turn_3.transform.position.x);
        ...
        tubeCat.transform.position = Turn_3.transform.position;

        Licat_yellow_ani.SetBool("is_tubeThrough", false);
        Licat_blue_ani.SetBool("is_tubeThrough", false);

        ChangeLevel_tube_2.closeCam(true, isBlueCat);
    }
